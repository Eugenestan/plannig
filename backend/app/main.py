...
def verify_api_key(request: Request, api_key: str = Form(...), email: str = Form(...)):
    """РџСЂРѕРІРµСЂСЏРµС‚ Рё СЃРѕС…СЂР°РЅСЏРµС‚ API РєР»СЋС‡ РІ СЃРµСЃСЃРёРё. РќР° РїСЂРѕРґРµ РЅРµ РґРѕР»Р¶РµРЅ РїР°РґР°С‚СЊ 500."""
    ...
    if not api_key or not email:
        return RedirectResponse(url="/?error=" + quote("Р—Р°РїРѕР»РЅРёС‚Рµ email Рё РєР»СЋС‡"), status_code=303)
    ...
    try:
        try:
            ...
            if test_response.status_code != 200:
                return RedirectResponse(url="/?error=" + quote(f"РќРµРєРѕСЂСЂРµРєС‚РЅС‹Р№ РєР»СЋС‡ РёР»Рё email (HTTP {test_response.status_code})"), status_code=303)
            ...
            import traceback
            print(f"verify-key error: {traceback.format_exc()}")
            db.rollback()
